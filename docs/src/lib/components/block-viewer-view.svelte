...
<div class="hidden group-data-[view=code]/block-view-wrapper:hidden lg:flex lg:h-(--height)">
	<div class="relative grid w-full gap-4">
		...
		<Resizable.PaneGroup direction="horizontal" class="relative z-10">
			...
			<Resizable.Handle
				class="relative z-20 hidden w-3 bg-transparent p-0 after:absolute after:end-0 after:top-1/2 after:h-8 after:w-1.5 after:-translate-x-px after:-translate-y-1/2 after:rounded-full after:bg-(--text)/24 after:transition-all hover:bg-transparent hover:after:h-10 focus-visible:bg-transparent lg:block"
			/>
