...
<TooltipPortal {...portalProps}>
	<TooltipPrimitive.Content
		...
		{side}
		class={cn(
			'z-50 w-fit max-w-xs origin-(--bits-tooltip-content-transform-origin) rounded-2xl bg-(--text) px-3 py-1.5 text-xs font-medium tracking-[-0.3px] text-(--bg) shadow-lg',
	...
	>
		...
		<TooltipPrimitive.Arrow>
			{#snippet child({ props })}
				<div
					class={cn(
						'z-50 size-2.5 rotate-45 rounded-[2px] bg-(--text)',
